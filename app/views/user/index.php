...
<html lang="en" class="ms-5 me-5">
    ...
    <body>
        ...
        <a href="<?php echo URL; ?>/user/create" class="btn btn-secondary mb-2 float-end">Tambah User</a>
            <table class="table table-striped">
                ...
                <tr>
                    <?php foreach ($data as $row) { 
                        $role = ($row['user_role']=="1")?"Admin":"User";
                        $aktif = ($row['user_aktif']=="1")?"Aktif":"Tidak Aktif";
                    ?>
                    <td class="text-center"><?= $row['user_id']?></td>
                    <td><?= $row['user_nama']?></td>
                    <td class="text-center"><?= $row['user_email']?></td>
                    <td class="text-center"><?= $row['user_password']?></td>
                    <td><?= $row['user_alamat']?></td>
                    <td class="text-center"><?= $row['user_pos']?></td>
                    <td class="text-center"><?= $row['user_hp']?></td>
                    <td class="text-center"><?= $role ?></td>
                    <td class="text-center"><?= $aktif ?></td>
                    <td class="text-center" style="width:50px;"><a href="<?= URL; ?>/user/edit/<?= $row['user_id']; ?>" class="badge text-bg-warning text-decoration-none">Edit</a></td>
                    <td class="text-center" style="width:50px;"><a href="<?= URL; ?>/user/delete/<?= $row['user_id']; ?>" class="badge text-bg-danger text-decoration-none" onclick="return confirm('Anda Yakin Ingin Menghapus Data Ini?');">Hapus</a></td>
                    </tr>
                    <?php }; ?>
            </table>
        </div>
